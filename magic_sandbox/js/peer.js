 ...
 * RouteCast and The Wolf Game carry. What changed, and nothing else:
 *
 *   - the namespace (KN -> MSN) and the peer-id prefixes (kn- -> msbx-,
 *     knc- -> msbxc-), so the apps cannot collide on a shared broker;
 *   - `window.MS_ICE` may replace the ICE list, as RouteCast's `RC_ICE` does,
 ...
 *
 * The room code *is* the host's peer id (msbx-<code>), so a guest needs no
...
(function (global) {
  ...
  function peerIdFor(code) {
    return "msbx-" + normalizeCode(code).toLowerCase();
  }
  ...
  function emitter(obj) {
    ...
    obj.emit = function (name) {
      ...
      (map[name] || []).slice().forEach(function (fn) {
        try { fn.apply(null, args); } catch (e) { console.error("[ms] handler", name, e); }
      });
    };
    return obj;
  }
  ...
  var wakeBus = (function () {
    ...
    function fire(reason, force) {
      ...
      subs.slice().forEach(function (fn) {
        try { fn(reason); } catch (e) { console.error("[ms] wake", e); }
      });
    }
  ...
  function Link(opts) {
    ...
    if (opts.initiator) {
      attach(pc.createDataChannel("ms", { ordered: true }));
    } else {
  ...
  /* ---------------- host side ----------------
   * Registers `msbx-<code>` on every broker simultaneously, so a guest that can
    ...
    function onSignal(broker, msg) {
      ...
      if (msg.type === "OFFER") {
        ...
        link
          ...
          .catch(function (e) {
            console.warn("[ms] answer failed", e);
  ...
  function join(code, handlers) {
    ...
    var connectionId = "ms_" + rid(10);   // stable across reconnects
    var gen = 0;
    ...
    function startAttempt() {
      ...
      a.broker.on("id-taken", function () {
        if (a.gen !== gen) return;
        a.myId = "msbxc-" + rid(10);   // ours to choose; just pick another
      });
