  ...
  global.WG.roles.define("diseased", {
    ...
    hooks: {
      onDeath: function (c) {
        ...
        c.out.say(killer.id, "🤒 You ate something that was already dying. It is in you now.", "death");
        c.R.kill(c.state, killer.id, { cause: "disease", byId: c.self.id, out: c.out, ignoreShields: true });
        return { public: "Whatever came through that window did not walk away from it." };
      },
      brief: function () {
