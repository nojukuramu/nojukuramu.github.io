/* ============================================================
   Hacks — noticing that a new version exists

   Lifted from Magic Sandbox (magic_sandbox/js/update.js), which lifted it
   from RouteCast (routecast/static/js/update.js) — one shape, on purpose, so
   every service worker in this repository answers the same "skip-waiting".
   Changes from Magic Sandbox: MS_VERSION becomes HK_VERSION, and the
   question asked before reloading is about a match in progress.
   ...
   match is in progress, reloading is confirmed first.
   ============================================================ */
...
export function version() { return window.HK_VERSION || "?"; }
...
export function apply() {
  // Reloading leaves the match you are in; your hacks, binds and settings are kept.
  if (busy() && !window.confirm("Reloading leaves the match you are in. Your hacks, binds and settings are kept. Update now?")) return;
  doApply();
}
