 ...
 *
 * Lifted wholesale from KaraokeNatin (`js/qr.js`), where it has already been
 * validated against a reference implementation and had its output read back by
 * OpenCV. Only the namespace changed (QR -> RC.qr), exactly as peer.js was
 * lifted before it. The conformance checks came with it — see tools/qr-test.js.
 *
 * No dependencies, no CDN. A room's invite link is about sixty characters,
 * which is a version 3 symbol at ECC M: small enough to stay sharp on a phone
 * held up at arm's length across a car park, and far inside what is here.
 * Throws if the payload does not fit version 25.
 *
 *   RC.qr.encode("https://example.com")  -> { size, modules }  (modules[y][x] = bool)
 *   RC.qr.draw(canvas, text, { scale, quiet, dark, light })
 */
(function (global) {
  "use strict";
...
if (typeof module !== "undefined" && module.exports) module.exports = globalThis.RC.qr;
