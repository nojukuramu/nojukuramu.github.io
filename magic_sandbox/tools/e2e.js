...
(async function main() {
  ...
  try {
    ...
    // drawing by hand, on a cleared fourth page, with Mirror off so counts are exact
    await page.click("#bookSlots .bslot:nth-child(4)");
    await page.click("#toolClear");
    await page.click("#toolMirror");
    const pg = await page.locator("#page").boundingBox();
    const pcx = pg.x + pg.width / 2, pcy = pg.y + pg.height / 2, ps = Math.min(pg.width, pg.height) / (2 * (160 + 70));
    const dot = (i) => { const a = -Math.PI / 2 + i * Math.PI / 6; return [pcx + Math.cos(a) * 160 * ps, pcy + Math.sin(a) * 160 * ps]; };
    const page4 = () => page.evaluate(() => window.MS.save.spell(3).layers[0].glyphs.map((g) => g.nodes.length));
    await page.click('[data-tool="glyph"]');
    await page.mouse.move(...dot(0)); await page.mouse.down();
    for (const n of [2, 5, 7, 10, 0]) await page.mouse.move(...dot(n), { steps: 6 });
    await page.mouse.up();
    check("dragging a pentagon through the dots draws Earth, not every dot it passes", JSON.stringify(await page4()) === "[5]", JSON.stringify(await page4()));
    for (const n of [6, 10, 2, 6]) await page.mouse.click(...dot(n));
    check("tapping dots one at a time draws a triangle", JSON.stringify(await page4()) === "[5,3]", JSON.stringify(await page4()));
    await page.mouse.move(...dot(0)); await page.mouse.down();
    for (const n of [1, 2, 3, 4, 5, 6]) await page.mouse.move(...dot(n), { steps: 4 });
    await page.mouse.up();
    check("a seven-sided trace is refused, with a reason", JSON.stringify(await page4()) === "[5,3]" && /3 to 6 sides/.test(await page.textContent("#pageHint")));
    await page.fill("#spellName", "Test Page");
    check("a name shows in the readout", (await page.textContent("#readout .rname b")) === "Test Page");
    await page.keyboard.press("Escape");
    check("Escape closes the book, even from the name field", await page.evaluate(() => document.getElementById("book").hidden) && await page.isVisible("#btnNew"));
    await page.keyboard.press("KeyB");
    check("B at the title opens the book above the menu", await page.evaluate(() => !document.getElementById("book").hidden && document.getElementById("scr-title").hidden));
