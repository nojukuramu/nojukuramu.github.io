 ...
 *
 * Lifted from Magic Sandbox's multiplayer (magic_sandbox/js/lobby.js on its
 * multiplayer branch), which keeps KaraokeNatin's room protocol. Changed, and
 * nothing else: the namespace (MSN -> HKN, MS_VERSION -> HK_VERSION), the
 * servers' names and codes, the storage keys and the word for a player.
 *
 ...
 *
 *   - Each server has a fixed code (RELY01, NODE02 …). Every code carries a
...
const NET = () => window.HKN.net;
export const version = () => String(window.HK_VERSION || "?");

export const SERVERS = [
  { code: "RELY01", name: "Relay",     color: "#39c6e8" },
  { code: "NODE02", name: "Node",      color: "#ff8a3d" },
  { code: "PING03", name: "Ping",      color: "#9b7bff" },
  { code: "HEAP04", name: "Heap",      color: "#6fdc7a" },
  { code: "ROOT05", name: "Root",      color: "#f2d35b" }
];
...
function emitter() {
  ...
  return {
    ...
    emit(n, a, b, c) { (map[n] || []).slice().forEach((fn) => { try { fn(a, b, c); } catch (e) { console.error("[hk] lobby " + n, e); } }); }
  };
}
...
function Directory(server) {
  ...
  function claim() {
    ...
    dropGuest(); dropHost();
    setState("connecting");
    const h = NET().host(server.code, { token: sessionToken("hacks:dir:" + server.code) });
    hostH = h;
    h.on("broker", (b) => {
...
export function hostRoom(opts) {
  ...
  const h = net.host(R.code, { token: sessionToken("hacks:room") });
  R.state = "open";
  ...
  h.on("guest-message", (link, m) => {
    ...
    if (m.t === "hello") {
      ...
      if (back) {
        ...
        R.roster.push({ id, name: cleanName(m.name, "Player " + id), team: pickTeam(R.roster), ready: false, host: false, away: false });
      }
      links.set(id, link);
...
export function joinRoom(code, name) {
  ...
  g.on("open", () => { if (!done) g.send({ t: "hello", name: cleanName(name, "Player"), v: version() }); });
...
function cleanRoster(a) {
  ...
  return a.slice(0, MAX_PLAYERS).map((p) => ({
    ...
    name: cleanName(p && p.name, "Player"),
    team: p && p.team === 1 ? 1 : 0,
