  ...
  WG.roles.define("cult_leader", {
    actions: {
      recruit: function (c) {
        ...
        c.out.say(c.occupant.id, "Someone is at your door.", "prompt", { promptId: id });
        c.out.say(c.actor.id, "Offer made. They are deciding.", "act");
        return { ok: true };
      }
    },
    hooks: {
      ...
      onConsent: function (c) {
        ...
        if (!c.ok) {
          c.out.say(leader.id, "" + target.name + " shut the door.", "warn");
        ...
        c.out.say(target.id, "You are a Cultist. Everything but the loyalty is unchanged.", "transform");
        c.out.say(leader.id, "" + target.name + " is one of ours.", "act");
        c.state.players.forEach(function (p) {
          if (p.alive && WG.roles.isCult(p.role) && p.id !== target.id && p.id !== leader.id) {
            c.out.say(p.id, "" + target.name + " has joined us.", "team");
          }
        });
        return null;
      },
      onDeath: function (c) {
        ...
        c.state.players.forEach(function (p) {
          if (p.alive && p.role === "fanatic") {
            ...
            promoted++;
            c.out.say(p.id, "The Leader is dead. You are Fanatic+ - you ward instead of dying.", "transform");
          }
        });
