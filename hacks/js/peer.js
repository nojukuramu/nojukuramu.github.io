 ...
 *
 * Lifted from KaraokeNatin (karaokenatin/js/peer.js), the same transport
 * RouteCast, The Wolf Game and Magic Sandbox's rooms carry, with exactly the
 * changes Magic Sandbox's multiplayer made to it and nothing else:
 *
 *   - the namespace (KN -> HKN) and the peer-id prefixes (kn- -> hkx-,
 *     knc- -> hkxc-), so the apps cannot collide on a shared broker;
 *   - `window.HK_ICE` may replace the ICE list, as RouteCast's `RC_ICE` does,
 *     so tools/mp-e2e.js can run two browsers on one machine with no STUN;
 *   - host() also reports *which* broker refused its id ("id-taken"). The
 *     game's servers are directories that whoever arrives first holds (see
 *     lobby.js), and "this id is already held on the first broker" is the
 *     fastest honest answer to "is somebody holding it?".
 *
 ...
 *
 * The room code *is* the host's peer id (hkx-<code>), so a guest needs no
...
(function (global) {
  ...
  function peerIdFor(code) {
    return "hkx-" + normalizeCode(code).toLowerCase();
  }
  ...
  function emitter(obj) {
    ...
    obj.emit = function (name) {
      ...
      (map[name] || []).slice().forEach(function (fn) {
        try { fn.apply(null, args); } catch (e) { console.error("[hk] handler", name, e); }
      });
    };
    return obj;
  }
  ...
  var wakeBus = (function () {
    ...
    function fire(reason, force) {
      ...
      subs.slice().forEach(function (fn) {
        try { fn(reason); } catch (e) { console.error("[hk] wake", e); }
      });
    }
  ...
  function Link(opts) {
    ...
    if (opts.initiator) {
      attach(pc.createDataChannel("hk", { ordered: true }));
    } else {
  ...
  /* ---------------- host side ----------------
   * Registers `hkx-<code>` on every broker simultaneously, so a guest that can
  ...
  function host(code, opts) {
    ...
    BROKERS.forEach(function (cfg) {
      ...
      b.on("open", function () {
        ...
        self.emit("broker", { host: cfg.host, up: true, index: BROKERS.indexOf(cfg) });
      });
      ...
      b.on("id-taken", function (round) {
        ...
        self.emit("id-taken", BROKERS.indexOf(cfg), round);
    ...
    function onSignal(broker, msg) {
      ...
      if (msg.type === "OFFER") {
        ...
        link
          ...
          .catch(function (e) {
            console.warn("[hk] answer failed", e);
  ...
  function join(code, handlers) {
    ...
    var connectionId = "hk_" + rid(10);   // stable across reconnects
    var gen = 0;
    ...
    function startAttempt() {
      ...
      a.broker.on("id-taken", function () {
        if (a.gen !== gen) return;
        a.myId = "hkxc-" + rid(10);   // ours to choose; just pick another
      });
